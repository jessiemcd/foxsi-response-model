...
FUNCTION add_sharing_new, distribution=distribution, threshold=threshold, $
	...
	if method EQ 'alssharing' then begin
		...
		if side EQ 'Pt' then begin
			;yellow = 0.12
			;USING 44 INSTEAD OF 45 FOR PT SIDE SO THAT THE CROSSING ZONES HAVE INTEGER WIDTH ON EACH SIDE
			censize=round(44)
		endif
	...
	for ei=0, ss[2]-1 do begin
		...
		if entry[0] GE stripbins/2. and entry[0] LT ps[1]-stripbins/2. then begin
			...
			if method EQ 'no_random_xs' or method EQ 'fixedfk' then begin
						...
						if next_strip EQ 0 then next_strip = -1
						next_strip+=hit_strip

						if method EQ 'fixedfk' then begin
							;Expressions derived from modified (correct) formula from Furukawa et al. 2020
							;Randomly assign two positions (x1, x2) within the hit strip and next strip.
							factor = -1
							while factor LT 0 or factor GE fix(stripbins) do begin
								;sample from normal distribution with sigma 0.4*strip width, 
								;centered at strip center
								factor = round(randomu(seed, /normal)*0.4*fix(stripbins)+float(stripbins)/2)
							endwhile
							x1 = hit_strip*fix(stripbins)+factor
							factor = -1
							;Now do it again for the other random position in the other strip
							while factor LT 0 or factor GE fix(stripbins) do begin
								factor = round(randomu(seed, /normal)*0.4*fix(stripbins)+float(stripbins)/2)
							endwhile
							x2 = next_strip*fix(stripbins)+factor
				
							;Calculate two energies based on the location + two random positions
							;Expressions derived from modified (correct) formula from Furukawa et al. 2020
							;Formula: X = (E1x1 + E2x2)/(E1+E2)
							E1 = entry[1]*(entry[0] - x2)/(x1 - x2)
							E2 = entry[1]*(entry[0] - x1)/(x2 - x1)
					
							;Eliminating cases with positions in the order X, x1, x2 (or opposite) 
							;- these give negative energies
							if E2 LE 0 then begin
								;print, 'E2<0'
								while E2 LE 0 do begin
									factor = -1
									while factor LT 0 or factor GE fix(stripbins) do begin
										factor = round(randomu(seed, /normal)*0.4*fix(stripbins)+float(stripbins)/2)
									endwhile
						...
						if method EQ 'no_random_xs' then begin
							;New version: (x1, x2) are always the strip centers.
							x1 = hit_strip*fix(stripbins)+float(stripbins)/2
							x2 = next_strip*fix(stripbins)+float(stripbins)/2
				
							if next_strip GT hit_strip then begin
								E2 = entry[1]*(entry[0] - x1)/(float(stripbins))
								E1 = entry[1]*(x2 - entry[0])/(float(stripbins))
							endif
							if next_strip LT hit_strip then begin
								E2 = entry[1]*(x1 - entry[0])/(float(stripbins))
								E1 = entry[1]*(entry[0] - x2)/(float(stripbins))
							endif
						endif
				
				
						;Note: In certain cases, X is closer to x2 than to x1. Therefore, strip 2 will get
						;the larger energy. This isn't in line with what actually occurs in the 
						;detector, and could be a motivator to change the method. 
						if E2 GT E1 then begin
							counter+=1
							;if abs(entry[0]-x2) LT abs(entry[0]-x1) then print, 'E2 Greater and x2 closer'
						endif
				
						er = float(E1)/float(E2)
						if finite(er) EQ 1 then energy_ratios = [energy_ratios, er]
			
						;Print statements to check behavior
			;			print, 'Hit: ', entry
			;			print, 'First Component: ', x1, E1
			;			print, 'Second Component: ', x2, E2
			;			print, 'Original Position: ', entry[0]
			;			print, 'Original Energy: ', entry[1]
			;			print, ''
			
						frame[0,0] = hit_strip
						frame[0,1] = next_strip
				
				
						if erez EQ 'erez_off' then begin
							;Add strip #, energy pairs to each frame ONLY IF ENERGY ABOVE THRESHOLD
							if E1 GT threshold then frame[1,0] = E1
							if E1 LE threshold then begin
								E1slost = [E1slost, E1]
								eventslost = [eventslost, (E1+E2)]
								if E2 GT threshold then begin
									eventslost1 = [eventslost, (E1+E2)]
									lostcomponents = [lostcomponents, E1]
									keptcomponents = [keptcomponents, E2]
								endif
							endif
				
							if E2 GT threshold then frame[1,1] = E2
							if E2 LE threshold then begin
								E2slost = [E2slost, E2]
								if E1 GT threshold then eventslost = [eventslost, (E1+E2)]
								if E1 GT threshold then begin
									eventslost1 = [eventslost, (E1+E2)]
									lostcomponents = [lostcomponents, E2]
									keptcomponents = [keptcomponents, E1]
								endif
							endif
						endif
						...
						if erez EQ 'erez_on' then begin
							...
							if E1+change GT threshold then frame[1,0] = E1+change
							change2 = randomu(seed, /normal)*0.51
							if E2+change2 GT threshold then frame[1,1] = E2+change2
					
							if E1+change LE threshold then begin
								E1slost = [E1slost, E1+change]
								if E2+change2 GT threshold then begin
									eventslost = [eventslost, (E1+change+E2+change2)]
									lostcomponents = [lostcomponents, E1+change]
									keptcomponents = [keptcomponents, E2+change2]
								endif
								frame[1,0] = 0.
							endif

							if E2+change2 LE threshold and E1+change GT threshold then begin
								E2slost = [E2slost, E2+change2]
								eventslost = [eventslost, (frame[1,0]+frame[1,1])]
								lostcomponents = [lostcomponents, frame[1,1]]
								keptcomponents = [keptcomponents, frame[1,0]]
								frame[1,1] = 0.
							endif
							;print, 'After EREZ:'
							;print, frame
						endif
						;if E1 LE threshold then print, 'WHy? E1', hit_strip, next_strip, E1, E2
						;if E2 LE threshold then print, 'WHy? E2', hit_strip, next_strip, E1, E2
						...
						if ogbins[-1] LT cross_wid or ogbins[-1] GE cross_wid+censize then begin
							...
							if entry[0] GE cross_wid and entry[0] LT ps[1]-cross_wid then begin
									...
									if erez EQ 'erez_on' then begin
										...
										if E1+change GT threshold then frame[1,0] = E1+change
										change = randomu(seed, /normal)*0.51
										if E2+change GT threshold then frame[1,1] = E2+change
									endif
	...
	;if using the fixedfx or no_random_xs methods, these lines can be used to check behavior
	;print, 'Number of E1s Lost', n_elements(E1slost)
	;print, 'Number of E2s Lost', n_elements(E2slost)
	
	;plot_hist, E2slost, bin=0.1
	;plot_hist, E1slost, bin=0.1, /oplot, linestyle=2
	
	;plot_hist, eventslost, bin=0.1
	;plot_hist, input_spectrum, bin=0.1, /oplot, linestyle=2
	
	;save, eventslost, eventslost1, lostcomponents, keptcomponents, input_spectrum, $
	;		filename='eventslost.sav'

	;se = size(events_list)
	;print, 'Number where E2>E1:', counter, '  Fraction of Events:', float(counter)/se[2]
...
FUNCTION remove_sharing_new, frame_list=frame_list, method=method, erez=erez, $
	...
	for i=0, s[3]-1 do begin
		...
		if n_elements(register) EQ 1 and register[0] NE -1 then begin
			...
			if method EQ 'fixedfk' or method EQ 'no_random_xs' then begin
	
					savr = f[0,1]
			
					if f[1,1] EQ 0 then begin
						while f[1,1] LE 0. or f[1,1] GT 4. do begin
							f[1,1] = randomu(seed)*4.
						endwhile
						which = RANDOMU(SEED, /NORMAL) GE 0
						if which EQ 0 and f[0,0] GT 0 then f[0,1] = f[0,0]-1
						if which EQ 1 or f[0,0] EQ 0 then f[0,1] = f[0,0]+1
					endif
			
					;print, f 
					;print, ''
		
					;This should not happen, thus the stop
					if f[1,0] EQ 0 then begin
						print, 'MAIN STRIP VALUE BELOW THRESHOLD!'
						;stop
						while f[1,0] LE 0. or f[1,0] GT 4. do begin
								f[1,0] = randomu(seed)*4.
						endwhile
					endif 
			...
			if method EQ 'alssharing' then begin
				...
				if randomu(seed, 1) LE yellow then begin
		...
		if n_elements(register) EQ 2 then begin
			...
			if method EQ 'fixedfk' or method EQ 'no_random_xs' then begin
				strips = ss[register]
				;print, 'Strips:', strips
				energies = es[register]
			
				;Let's arrange to always have the higher energy event first
				if energies[0] LT energies[1] then begin
					energies = [energies[1], energies[0]]
					strips = [strips[1], strips[0]]
				endif
					
				;Solve for reconstructed position, X
				if method EQ 'fixedfk' then begin
					;randomly assign positions within each strip
					strips = ss[register]
					factor = -1
					while factor LT 0 or factor GE stripbins do begin
						factor = round(randomu(seed, /normal)*0.4*stripbins+stripbins/2.)
					endwhile
					x1 = strips[0]*stripbins+factor
					x1s=[x1s, factor]
					factor = -1
					while factor LT 0 or factor GE stripbins do begin
						factor = round(randomu(seed, /normal)*0.4*stripbins+stripbins/2.)
					endwhile
					x2 = strips[1]*stripbins+factor
					x2s=[x2s, factor]
				
					;print, x1, x2
					if (x1/fix(stripbins)) EQ (x2/fix(stripbins)) then print, 'SAME STRIP', x1, x2	
			
					X = (energies[0]*x1 + energies[1]*x2)/(energies[0]+energies[1])
				endif
				...
				;print,''
				if method EQ 'no_random_xs' then begin
					if strips[0] GT strips[1] then begin
						;print, 'hi'
						X = -float(stripbins)*energies[1]/(energies[0]+energies[1]) + float(stripbins)*(strips[0]+0.5)
					endif
					if strips[1] GT strips[0] then begin
						;print, 'hey'
						X = float(stripbins)*energies[1]/(energies[0]+energies[1]) + float(stripbins)*(strips[0]+0.5)
					endif
				endif
				...
				X = round(X)
				;if X mod 10 GE 7 then begin
				double_bins = [double_bins, (X mod stripbins)]
				;Add reconstructed position to hit list + reconstructed energy to energy list
				hit_list = [hit_list, X]
				spectrum = [spectrum, (energies[0]+energies[1])]
			endif
...
pro response_modeling, source=source, strips=strips, threshold=threshold, $
						method=method, erez=erez, edist=edist, $
...
default, edist, 'ALS_high'
default, alssharing, 0
...
frame_list = add_sharing_new(distribution=phi_psf, threshold=threshold, $
...
if keyword_set(PLOTSPECTRA) then begin
	...
	plot, ss_x, ss, title='Reconstructed Spectrum', linestyle=6, psym=4, $
			ytitle='Counts/bin', xthick=th, ythick=th, font=-1, charthick=fth, charsize=2, $
			yrange=[0,1.2*max(ss)], ystyle=1, symsize=0.1, xtitle='Energy (keV)', xtickinterval=2
	for n=0, n_elements(ss)-1 do begin
		oplot, [ss_x[n],ss_x[n]], [0, ss[n]], thick=th
		oplot, [ss_x[n],ss_x[n]]+binn, [0, ss[n]], thick=th
		oplot, [ss_x[n], ss_x[n]+binn], [ss[n], ss[n]], thick=th
	endfor
	oplot, [threshold, threshold], [0, 10000], color=50, thick=th
	...
	plot, cs_x, cs, title='Reconstructed Spectrum - No Add Back', linestyle=6, psym=4, $
